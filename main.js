...
function initGL() {
    ...
    audioSurface.BufferData(
        CreateSphereSurface(),
        new Array(CreateSphereSurface().length).fill(1)
    )*/
...
function LoadTexture() {
    ...
    image.src = "https://raw.githubusercontent.com/ChyzhykNazar/MSVR/main/texture.jpeg";
    image.onload = () => {
        gl.bindTexture(gl.TEXTURE_2D, texture);
        gl.texImage2D(
            gl.TEXTURE_2D,
...
function initAudio() {
    ...
    audio.addEventListener('play', () => {
        if (!context) {
            ...
            biquadFilter.frequency.value = 999;
